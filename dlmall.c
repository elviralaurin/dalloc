...
#define HEAD_SIZE    (sizeof(Taken))  // 24 bytes

// the minimum size block (not including the header) we will hand out is now 8 bytes
...
#define MAGIC(memory)   ((Taken*)memory - 1)
#define HIDE(block)    (void*)((Taken*)block + 1)
